...
private:
    typedef google::sparse_hash_set<Key, KeyHash> hash_type;

public:
    SparseSet() { init_keys(); }
    SparseSet(const SparseSet & hash) = default;
    SparseSet(SparseSet && hash) = default;
    SparseSet(size_t size) : m_hash(size) { init_keys(); }

    SparseSet & operator=(const SparseSet & hash) = default;
    SparseSet & operator=(SparseSet && hash) = default;
    ...
    bool operator==(const SparseSet & hash) const { return m_hash == hash.m_hash; }
    bool operator!=(const SparseSet & hash) const { return !operator==(hash); }

    void clear() { m_hash.clear(); }
    void erase(iterator pos) { m_hash.erase(pos); }
    size_t erase(const KeyT & key) { return m_hash.erase(key); }

    void swap(SparseSet & hash) { m_hash.swap(hash.m_hash); }
...
public:
    ...
    friend std::ostream & operator<<(std::ostream & strm, const SparseSet & hash) { return hash.Print(strm); }
