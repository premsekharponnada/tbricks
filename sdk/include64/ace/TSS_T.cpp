...
ACE_TSS<TYPE>::~ACE_TSS (void)
{
  ...
  bool once;
  ATOMIC_LOAD_RELAXED(once, &this->once_);
  if (once)
  {
...
ACE_TSS<TYPE>::ts_init (void)
{
  ...
  // once!
  bool once;
  ATOMIC_LOAD_ACQUIRE(once, &this->once_);
  if (!once)
    {
      if (ACE_Thread::keycreate (&this->key_,
      ...
      else
        {
          ...
          ATOMIC_STORE_RELEASE(&this->once_, true);
          return 0;
        }
    }
...
ACE_TSS<TYPE>::ts_object (void) const
{
  bool once;
  ATOMIC_LOAD_RELAXED(once, &this->once_);
  if (!once) // Return 0 if we've never been initialized.
    return 0;
