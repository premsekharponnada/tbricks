...
ACE_Singleton<TYPE, ACE_LOCK>::instance (void)
{
  ...
  ACE_Singleton<TYPE, ACE_LOCK> *&singleton_ref =
    ACE_Singleton<TYPE, ACE_LOCK>::instance_i ();

  ACE_Singleton<TYPE, ACE_LOCK> *singleton;
  ATOMIC_LOAD_ACQUIRE(singleton, &singleton_ref);
  ...
  if (singleton == 0)
    {
      if (ACE_Object_Manager::starting_up () ||
          ACE_Object_Manager::shutting_down ())
        {
          ...
          ACE_NEW_RETURN (singleton_ref, (ACE_Singleton<TYPE, ACE_LOCK>), 0);
      ...
      else
        {
          ...
          // can be volatile access or RELAXED
          ATOMIC_LOAD_ACQUIRE(singleton, &singleton_ref);
          if (singleton == 0)
            {
#endif /* ACE_MT_SAFE */
              ...
              ATOMIC_STORE_RELEASE(&singleton_ref, singleton);
#if defined (ACE_MT_SAFE) && (ACE_MT_SAFE != 0)
            }
#endif /* ACE_MT_SAFE */
        }
...
ACE_TSS_Singleton<TYPE, ACE_LOCK>::instance (void)
{
  ...
  ACE_TSS_Singleton<TYPE, ACE_LOCK> *&singleton_ref =
    ACE_TSS_Singleton<TYPE, ACE_LOCK>::instance_i ();
  ...
  if (singleton == 0)
    {
      if (ACE_Object_Manager::starting_up () ||
          ACE_Object_Manager::shutting_down ())
        {
          ...
          ACE_NEW_RETURN (singleton_ref, (ACE_TSS_Singleton<TYPE, ACE_LOCK>), 0);
          singleton = singleton_ref;
        }
      else
        {
